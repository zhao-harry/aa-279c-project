...
\begin{align*}
    \Vec{M_S} = \sum^{n}_{i=1} \Vec{r_i} \times e_i \int_{S_i} \Vec{df_{total_i}} \\
    \Vec{df_{total}} = -P 2 ((1-C_S) \hat{\Vec{S}} + (C_S \cos{\theta} + \frac{1}{3} C_d) \cos{\theta} dA
\end{align*}

The equations below define the aerodynamic torque.
\begin{align*}
    \Vec{df_{aero}} = - \frac{1}{2} C_D \rho V^2 (\hat{\Vec{V}} \dot \hat{\Vec{V}}
\end{align*}
