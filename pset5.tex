...
\begin{align*}
    \Vec{M_m} = \Vec{m_{sat}} \times \Vec{B_{Earth}}
\end{align*}
...
\begin{align*}
    \Vec{M_S} = \sum^{n}_{i=1} \Vec{r_i} \times e_i \int_{S_i} d \Vec{f}_{total_i} \\
    d \Vec{f}_{total} = -P ((1-C_S) \Vec{\hat{S}} + 2 (C_S \cos{\theta} + \frac{1}{3} C_d) \cos{\theta} dA
\end{align*}

The equations below define the aerodynamic torque.
\begin{align*}
    d \Vec{f}_{aero} = - \frac{1}{2} C_D \rho V^2 (\Vec{\hat{V}} \cdot \Vec{\hat{N}}) \Vec{\hat{V}} dA
\end{align*}
