function [B_R, B_theta, B_phi] = magFieldEarth(R, phi, theta, RE)
    ...
    for n = 1:4
        ...
        for mInd = 1:n
            ...
            BR_temp = BR_temp + (g(n,mInd)*cos(m*phi) + h(n,mInd)*sin(m*phi))*P_nm;
            BTheta_temp = BTheta_temp + (g(n,mInd)*cos(m*phi)+ h(n,mInd)*sin(m*phi))*dPnm_dtheta;
            BPhi_temp = BPhi_temp + m*(-g(n,mInd)*sin(m*phi) + h(n,mInd)*cos(m*phi))*P_nm;
        end
